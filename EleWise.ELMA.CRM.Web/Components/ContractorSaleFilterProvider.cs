...
// Type: EleWise.ELMA.CRM.Web.Components.ContractorSaleFilterProvider
...
namespace EleWise.ELMA.CRM.Web.Components
{
  ...
  public class ContractorSaleFilterProvider : IEntityFilterProvider
  {
    ...
    public Guid ProviderUid => ContractorSaleFilterProvider.UID;

    public void ApplyFilter(IEntityFilter filter, string data)
    {
      if (string.IsNullOrEmpty(data) || !(filter is ISaleFilter saleFilter))
        return;
      saleFilter.Contractors.Add(ContractorManager.Instance.LoadOrNull((long) int.Parse(data)));
