...
// Type: EleWise.ELMA.CRM.Web.Components.SaleTypeFilterProvider
...
namespace EleWise.ELMA.CRM.Web.Components
{
  ...
  public class SaleTypeFilterProvider : IEntityFilterProvider
  {
    ...
    public Guid ProviderUid => SaleTypeFilterProvider.UID;
    ...
    public IEntityFilter GetFilter(Guid uid, IEntityFilter filter)
    {
      try
      {
        return filter is ISaleTypeFilter saleTypeFilter ? (IEntityFilter) saleTypeFilter : (IEntityFilter) InterfaceActivator.Create(typeof (ISaleTypeFilter));
      }
      catch
      {
      }
