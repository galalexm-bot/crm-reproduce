...
public class StartMenuTreeItem : MenuTreeItem
{
	public enum AddEnum
	{
		Allow,
	...
	public override string GetEncodedText()
	{
		...
		base.HtmlAttributes = base.HtmlAttributes ?? new Dictionary<string, object>();
		base.HtmlAttributes["level"] = Level;
		if (Level == 1)
		{
			base.HtmlClass = base.HtmlClass ?? new List<string>();
			base.HtmlClass.Add("Group_" + Color);
			base.HtmlAttributes["row"] = Row;
			base.HtmlAttributes["add"] = Add.ToString().ToLower();
		}
		...
		if (Level == 1 && (Add == AddEnum.Allow || Add == AddEnum.AllowRow))
		{
		...
		if (Array.Exists(MenuManager.SystemMenus, (Guid uid) => base.Menu.Uid == uid))
		{
