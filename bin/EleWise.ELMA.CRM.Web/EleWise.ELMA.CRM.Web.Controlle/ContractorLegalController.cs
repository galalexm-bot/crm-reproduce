...
public class ContractorLegalController : BPMController<IContractorLegal, long>
{
	...
	[ContractorCreate]
	[Permission("FED901DB-3E4A-4BF0-B3DD-5BD1CC0D7255", typeof(IContractorLegal))]
	public ActionResult Create()
	{
		ContractorLegalModel contractorLegalModel = new ContractorLegalModel();
		contractorLegalModel.LockEditPermission = ComplexCacheService.GetOrAdd("A0EEA553-0D17-4a09-A1A3-8BD23C38FA8E", "LockNameReroutePermissionTimestamp", () => new CachePermissionModel(blocking: false)).Blocking;
		if (contractorLegalModel.LockEditPermission)
		{
		...
		InitNewModel(contractorLegalModel);
		return (ActionResult)(object)((Controller)this).View((object)contractorLegalModel);
	}

	[EntityForm("Create", "EleWise.ELMA.CRM.Web")]
	[Permission("FED901DB-3E4A-4BF0-B3DD-5BD1CC0D7255", typeof(IContractorLegal))]
	public ActionResult CreatePopup([ParameterContainer] IContractorLegal contractor, Guid? formUid = null, ViewType? viewType = null, bool formReadOnly = false, string parentFormId = "", string formId = "")
	{
		ContractorLegalModel contractorLegalModel = new ContractorLegalModel(contractor, (!viewType.HasValue) ? ViewType.Create : viewType.Value)
		{
			FormUid = formUid,
			FormReadOnly = formReadOnly,
			FormId = formId
		};
		InitNewModel(contractorLegalModel, contractor == null);
		contractorLegalModel.DynamicFormSettings.ParentFormId = parentFormId;
		DynamicFormSettings dynamicFormSettings = contractorLegalModel.DynamicFormSettings;
		dynamicFormSettings.ViewItemId = dynamicFormSettings.ViewItemId + ";" + contractorLegalModel.View.Uid;
		return (ActionResult)(object)((Controller)this).PartialView("ContractorLegal/ContractorLegalEditForm", (object)contractorLegalModel);
	}

	[HttpPost]
	[Permission("FED901DB-3E4A-4BF0-B3DD-5BD1CC0D7255", typeof(IContractorLegal))]
	[ContentItem(Name = "SR.M('Создание юридического лица')", Image24 = "#add.svg")]
	[TransactionAction]
	public ActionResult Create(ContractorLegalModel model)
	{
		...
		{
			text = (IsCreatorRequest() ? "" : SR.T("Юридическое лицо <a href=\"{1}\">{0}</a> сохранено успешно", model.Entity.Name.HtmlEncode(), ((Controller)this).get_Url().Entity(model.Entity)));
			if (saveWithCategoryRulesResult.AddedCategories.Count > 0)
			{
				if (!IsCreatorRequest())
				{
				...
				{
					text = SR.T("Юридическому лицу {0} автоматически добавлены категории: {1}", model.Entity.Name, saveWithCategoryRulesResult.AddedCategories.Select((ICategory c) => c.Name).ToSeparatedString(", ")).HtmlEncode();
				}
			}
	...
	public ActionResult Details(long id, int? tabIndex = null)
	{
		try
		{
			if (tabIndex.HasValue && DirectoryMapVirtualPathProvider.AnyConfigFile("~/Modules/EleWise.ELMA.CRM.Web/Views/ContractorLegal/TabView.cshtml", "~/Modules/EleWise.ELMA.CRM.Web/Views/Shared/Contractor/TasksTab.cshtml"))
			{
				((ControllerBase)this).get_ViewData().set_Item("SelectedTab", (object)tabIndex.Value);
			}
			IContractorLegal contractorLegal = base.Manager.Load(id);
			ThrowExceptionIfDeleted(contractorLegal);
			ContractorLegalModel contractorLegalModel = new ContractorLegalModel(contractorLegal, ViewType.Display);
			contractorLegalModel.LockEditPermission = ComplexCacheService.GetOrAdd("A0EEA553-0D17-4a09-A1A3-8BD23C38FA8E", "LockNameReroutePermissionTimestamp", () => new CachePermissionModel(blocking: false)).Blocking;
			return (ActionResult)(object)((Controller)this).View((object)contractorLegalModel);
		}
	...
	{
		ContractorLegalModel contractorLegalModel = new ContractorLegalModel(base.Manager.LoadOrCreate(id), viewType);
		contractorLegalModel.FormUid = formUid;
		contractorLegalModel.LockEditPermission = ComplexCacheService.GetOrAdd("A0EEA553-0D17-4a09-A1A3-8BD23C38FA8E", "LockNameReroutePermissionTimestamp", () => new CachePermissionModel(blocking: false)).Blocking;
		return AsyncViewItemView(contractorLegalModel.View, viewType, viewItemUid, contractorLegalModel, (ContractorLegalModel m) => m.Entity);
	}
	...
	public ActionResult Edit(long id)
	{
		try
		{
			IContractorLegal contractorLegal = base.Manager.Load(id);
			ThrowExceptionIfDeleted(contractorLegal);
			if (!base.SecurityService.HasPermission(CRMPermissionProvider.EditContractorPermission, contractorLegal))
			{
				throw new SecurityException(SR.T("Недостаточно прав для редактирования контрагента \"{0}\" с идентификатором {1}", contractorLegal.Name, contractorLegal.Id));
			}
			ContractorLegalModel contractorLegalModel = new ContractorLegalModel(contractorLegal, ViewType.Edit);
			contractorLegalModel.LockEditPermission = ComplexCacheService.GetOrAdd("A0EEA553-0D17-4a09-A1A3-8BD23C38FA8E", "LockNameReroutePermissionTimestamp", () => new CachePermissionModel(blocking: false)).Blocking;
			if (contractorLegalModel.LockEditPermission)
			{
	...
	[HttpPost]
	[TransactionAction]
	public ActionResult Edit(ContractorLegalModel model)
	{
		try
		{
			model.LockEditPermission = ComplexCacheService.GetOrAdd("A0EEA553-0D17-4a09-A1A3-8BD23C38FA8E", "LockNameReroutePermissionTimestamp", () => new CachePermissionModel(blocking: false)).Blocking;
			...
			{
				string text = SR.T("Юридическое лицо <a href=\"{1}\">{0}</a> сохранено успешно", model.Entity.Name.HtmlEncode(), ((Controller)this).get_Url().Entity(model.Entity));
				if (saveWithCategoryRulesResult.AddedCategories.Count > 0)
				{
	...
	public ActionResult Delete(long id)
	{
		try
		{
			...
			IContractorLegal contractorLegal = base.Manager.Load(id);
			base.Manager.Delete(contractorLegal);
			base.Notifier.Information(SR.T("Юридическое лицо \"{0}\" успешно удалено", contractorLegal.Name));
			return (ActionResult)(object)((Controller)this).RedirectToAction("Index", "Contractor", (object)new
			{
				area = "EleWise.ELMA.CRM.Web"
			});
	...
	protected void InitNewModel(ContractorLegalModel model)
	{
		InitNewModel(model, contractorWasNull: true);
	}

	private void InitNewModel(ContractorLegalModel model, bool contractorWasNull)
	{
