...
public abstract class RelationshipAddDayMethodExecutor<T> : PublicServiceMethodExecutor where T : class, IRelationship
{
	...
	public RelationshipAddDayMethodExecutor(WebData parameters)
		: base(parameters)
	{
	}
	...
	private static void DoCompleteAction(long entityId, string comment)
	{
		...
		try
		{
			RelationshipManager.Instance.AddDay(entityId, comment);
		}
		catch (Exception)
		{
			unitOfWork.Rollback();
