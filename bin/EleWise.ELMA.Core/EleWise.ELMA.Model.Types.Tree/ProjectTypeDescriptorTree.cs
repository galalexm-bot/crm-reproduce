...
using EleWise.ELMA.Api.Services.Internal.Projects;
...
[Component(Order = 399)]
internal sealed class ProjectTypeDescriptorTree : EntityDescriptorTree
{
	...
	private struct _003CGetSubTypes_003Ed__3 : IAsyncStateMachine
	{
		...
		public ProjectTypeDescriptorTree _003C_003E4__this;
		...
		private void MoveNext()
		{
			...
			ProjectTypeDescriptorTree projectTypeDescriptorTree = _003C_003E4__this;
			...
			try
			{
				...
				if (num != 0)
				{
					...
					_003CmetadataList_003E5__2 = Enumerable.ToArray<MetadataHeader>(projectTypeDescriptorTree.GetSubTypes(filterModel));
					...
					val2 = projectTypeDescriptorTree.projectMetadataHeadService.GetActiveProjectTypeUids().GetAwaiter();
				...
				goto IL_00fe;
				IL_00fe:
				val = CompositeTypeDescriptorHelper.GetDescriptorsByGroups(typeDescriptor, _003CmetadataList_003E5__2, projectTypeDescriptorTree.metadataItemGroupManager, projectTypeDescriptorTree.metadataService).GetAwaiter();
	...
	private readonly IProjectMetadataHeadService projectMetadataHeadService;

	public ProjectTypeDescriptorTree(IMetadataServiceContext metadataService, MetadataItemGroupManager metadataItemGroupManager, IProjectMetadataHeadService projectMetadataHeadService)
		: base(metadataService, metadataItemGroupManager)
	{
		this.projectMetadataHeadService = projectMetadataHeadService;
	}

	public override bool CanBuildTree(ITypeDescriptor typeDescriptor)
	{
		return typeDescriptor is ProjectTypeDescriptor;
	}
	...
	{
		//IL_0007: Unknown result type (might be due to invalid IL or missing references)
		return EntityDescriptorTree.GetSubTypes(metadataService, filterModel, MetadataConstants.ProjectMetadataTypeUid);
	}
}
