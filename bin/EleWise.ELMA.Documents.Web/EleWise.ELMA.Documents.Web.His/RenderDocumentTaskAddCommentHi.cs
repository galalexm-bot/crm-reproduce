...
[Component]
public class RenderDocumentTaskAddCommentHistory : RenderInDocumentHistoryBase
{
	...
	{
		string caption = ((@event.New != null) ? SR.T("Добавлен комментарий к задаче \"{0}\"", $"<a href=\"{html.Url().Entity(@event.New)}\">{SR.T(((ITaskBase)@event.New).Subject.HtmlEncode())}</a>") : null);
