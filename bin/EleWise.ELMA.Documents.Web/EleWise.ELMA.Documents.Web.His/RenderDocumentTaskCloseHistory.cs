...
[Component]
public class RenderDocumentTaskCloseHistory : RenderInDocumentHistoryBase
{
	...
	{
		string caption = ((@event.New != null) ? SR.T("Задача \"{0}\" закрыта", $"<a href=\"{html.Url().Entity(@event.New)}\">{SR.T(((ITaskBase)@event.New).Subject.HtmlEncode())}</a>") : null);
		ApplySettings(@event, historyLoader);
		return TaskBaseSimpleEventRender.RenderCloseTask(html, @event, historyLoader, caption);
	}
