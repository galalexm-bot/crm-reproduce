...
[Component]
public class RenderDocumentTaskControlCompleteHistory : RenderInDocumentHistoryBase
{
	...
	{
		string caption = ((@event.New != null) ? SR.T("Контроль выполнения задачи \"{0}\" завершен", $"<a href=\"{html.Url().Entity(@event.New)}\">{SR.T(((ITaskBase)@event.New).Subject.HtmlEncode())}</a>") : null);
