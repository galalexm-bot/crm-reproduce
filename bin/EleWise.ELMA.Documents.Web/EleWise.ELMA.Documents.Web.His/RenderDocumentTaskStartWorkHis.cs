...
[Component]
public class RenderDocumentTaskStartWorkHistory : RenderInDocumentHistoryBase
{
	...
	{
		string caption = ((@event.New != null) ? SR.T("Начата работа по задаче \"{0}\"", $"<a href=\"{html.Url().Entity(@event.New)}\">{SR.T(((ITaskBase)@event.New).Subject.HtmlEncode())}</a>") : null);
