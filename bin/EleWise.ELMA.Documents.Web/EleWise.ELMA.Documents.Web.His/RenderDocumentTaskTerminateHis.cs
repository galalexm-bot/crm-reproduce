...
[Component]
public class RenderDocumentTaskTerminateHistory : RenderInDocumentHistoryBase
{
	...
	{
		string caption = ((@event.New != null) ? SR.T("Задача \"{0}\" прервана", $"<a href=\"{html.Url().Entity(@event.New)}\">{SR.T(((ITaskBase)@event.New).Subject.HtmlEncode())}</a>") : null);
		ApplySettings(@event, historyLoader);
		return TaskBaseSimpleEventRender.RenderTerminateTask(html, @event, historyLoader, caption);
	}
