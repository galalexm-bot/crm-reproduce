...
internal sealed class ClientComponentCodeEditorExtension : AbstractComponentCodeEditorExtension
{
	private readonly DbPreUpdater dbPreUpdater;

	protected override Guid ModuleTypeUid => ComponentClientScriptModuleType.TypeUid;

	protected override CodeType CodeType => CodeType.Client;

	public ClientComponentCodeEditorExtension(IRuntimeApplication runtimeApplication)
	{
	...
	public override IScriptData GenerateSource(InitWorkspaceRequest request)
	{
		...
		metadataSourcesProviderArgs.ClientScriptModule = scriptModule;
		metadataSourcesProviderArgs.ViewScriptModule = AbstractCodeEditorExtension.FindScriptModule(request.ScriptModules, ComponentViewScriptModuleType.TypeUid);
	...
	private IList<IScriptSource> GenerateSources(MetadataSourcesProviderArgs args)
	{
		...
		{
			return base.ComponentMetadataSourcesService.GetClientSources(args).ToList();
		}
	}
