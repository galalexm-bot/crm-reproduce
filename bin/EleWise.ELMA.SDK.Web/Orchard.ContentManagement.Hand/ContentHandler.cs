...
public abstract class ContentHandler : IContentHandler, IDependency
{
	private class InlineStorageFilter<TPart> : StorageFilterBase<TPart> where TPart : class, IContent
	{
	...
	private class InlineTemplateFilter<TPart> : TemplateFilterBase<TPart> where TPart : class, IContent
	{
	...
	{
		Filters.Add(new InlineStorageFilter<TPart>
		{
			OnActivated = handler
		});
	}
	...
	{
		Filters.Add(new InlineTemplateFilter<TPart>
		{
			OnGetItemMetadata = handler
		});
	}
	...
	protected void OnUpdateEditorShape<TPart>(Action<UpdateEditorContext, TPart> handler) where TPart : class, IContent
	{
		Filters.Add(new InlineTemplateFilter<TPart>
		{
			OnUpdateEditorShape = handler
		});
	}

	void IContentHandler.Activating(ActivatingContentContext context)
	{
		foreach (IContentActivatingFilter item in Filters.OfType<IContentActivatingFilter>())
		{
			item.Activating(context);
		}
		Activating(context);
	}
	...
	{
		foreach (IContentStorageFilter item in Filters.OfType<IContentStorageFilter>())
		{
			item.Activated(context);
		}
		Activated(context);
	}
	...
	void IContentHandler.ImportCompleted(ImportContentContext importContentContext)
	{
		foreach (IContentStorageFilter item in Filters.OfType<IContentStorageFilter>())
		{
			item.ImportCompleted(importContentContext);
		}
	...
	{
		foreach (IContentTemplateFilter item in Filters.OfType<IContentTemplateFilter>())
		{
			item.GetContentItemMetadata(context);
		}
	...
	void IContentHandler.BuildDisplay(BuildDisplayContext context)
	{
		foreach (IContentTemplateFilter item in Filters.OfType<IContentTemplateFilter>())
		{
			item.BuildDisplayShape(context);
		}
	...
	void IContentHandler.UpdateEditor(UpdateEditorContext context)
	{
		foreach (IContentTemplateFilter item in Filters.OfType<IContentTemplateFilter>())
		{
			item.UpdateEditorShape(context);
		}
