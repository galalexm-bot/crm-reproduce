...
public class TaskAddQuestionMethodExecutor : PublicServiceMethodExecutor
{
	public const string MethodName = "AddQuestion";

	protected long QuestionId;

	public static string Description => SR.T("Задать вопрос по задаче");
	...
	{
		b.Name("TaskId").Descriptor(SR.T("Идентификатор задачи, по которой задается вопрос (Int64)"));
	...
	public TaskAddQuestionMethodExecutor(WebData parameters)
		: base(parameters)
	{
	}
	...
	protected virtual WebData ExecuteInternal()
	{
		...
		DoAddQuestionAction(taskBase, executor, value, attachments, result3);
		return GetResult();
	}
	...
	public virtual void DoAddQuestionAction(ITaskBase taskBase, IUser executor, string description, IList<IAttachment> attachments, bool showAll)
	{
		...
		try
		{
			...
			question.Subject = taskBase.ToString();
			...
			question.Save();
			EntityActionEventArgs entityActionEventArgs = EntityActionEventArgs.TryCreate(null, taskBase, TaskBaseActions.AddQuestionGuid);
		...
		{
			unitOfWork.Rollback();
			throw PublicServiceException.CreateWebFault(SR.T("При добавлении вопроса возникла ошибка"), innerException, 500);
		}
		unitOfWork.Commit();
	}
