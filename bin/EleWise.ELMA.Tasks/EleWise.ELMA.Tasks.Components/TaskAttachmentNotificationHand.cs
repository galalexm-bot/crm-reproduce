...
[Component]
internal class TaskAttachmentNotificationHandler : INotificationMessageHandler, IEventHandler
{
	public void ProcessNotificationToMessage(INotification notification, IMessage message)
	{
		...
		if (typeByUidOrNull == null || !typeof(ITaskBase).IsAssignableFrom(typeByUidOrNull) || !long.TryParse(message.ObjectId, out var mId))
		{
			return;
		}
		IEntity entity = null;
		...
		if (!(entity is ITaskBase))
		{
			return;
		}
		ITaskBase obj = (ITaskBase)entity;
		List<string> list = new List<string>();
		double num = 0.0;
		foreach (IAttachment item in (IEnumerable<IAttachment>)obj.Attachments)
		{
			if (item.File != null)
			{
