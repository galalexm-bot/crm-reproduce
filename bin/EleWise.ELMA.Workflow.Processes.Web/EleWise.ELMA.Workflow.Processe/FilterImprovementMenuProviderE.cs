...
[Component(Order = 80)]
public class FilterImprovementMenuProviderExtension : IFilterMenuProviderExtension
{
	...
	{
		if (!(filterFolder.Uid == WorkflowConstants.ImprovementProcessesActionFilter))
		{
			return filterFolder.Uid == WorkflowConstants.ImprovementAllProcessesActionFilter;
		}
		return true;
	}
	...
	public void GetTreeNodeParameters(IFilterFolder filterFolder, UrlHelper urlHelper, bool isDefault, bool isSystemDefault, out string type, out string icon, out string href, out bool canMakeDefault, ActionRoute routes, string filterIdParameter = null, string urlAction = null)
	{
		...
		if (urlHelper != null)
		{
			href = urlHelper.Action((filterFolder.Uid == WorkflowConstants.ImprovementProcessesActionFilter) ? "Processes" : "AllProcesses", "Improvement", (object)new
			{
		...
		{
			type = "action";
			icon = (isDefault ? "#link.svg" : (isSystemDefault ? "#link.svg" : "#arrow_up.svg"));
		}
	}
}
