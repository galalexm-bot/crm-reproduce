...
[Component(Order = 80)]
public class FilterMyProcessMenuProviderExtension : IFilterMenuProviderExtension
{
	...
	{
		return filterFolder.Uid == WorkflowConstants.MyProcessesActionFilter;
	}
	...
	public void GetTreeNodeParameters(IFilterFolder filterFolder, UrlHelper urlHelper, bool isDefault, bool isSystemDefault, out string type, out string icon, out string href, out bool canMakeDefault, ActionRoute routes, string filterIdParameter = null, string urlAction = null)
	{
		...
		if (urlHelper != null)
		{
			href = urlHelper.Action("MyProcesses", "ProcessHeader", (object)new
			{
		...
		{
			type = "action";
			icon = (isDefault ? "#link.svg" : (isSystemDefault ? "#link.svg" : "#process.svg"));
		}
	}
}
