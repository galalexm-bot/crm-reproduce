...
internal class Compiler
{
	...
	internal static string GetTempAssemblyPath(string baseDir, Assembly[] assemblies, string defaultNamespace)
	{
		if (assemblies == null)
		{
			throw new ArgumentNullException("assemblies");
		}
		if (assemblies.Any((Assembly a) => a.IsDynamic))
		{
		...
		try
		{
			...
			string text = ((assemblies.Length == 1) ? GetTempAssemblyName(assemblies[0].GetName(), defaultNamespace) : ("Serializers_" + Guid.NewGuid().ToString("n")));
			baseDir = ((!baseDir.EndsWith("\\", StringComparison.Ordinal)) ? (baseDir + "\\" + text) : (baseDir + text));
		}
		finally
		{
	...
	internal Assembly Compile(Assembly[] parentAssemblies, string ns, XmlSerializerCompilerParameters xmlParameters, Evidence evidence)
	{
		...
		permissionSet.Assert();
		if (parentAssemblies != null && (codeDomParameters.OutputAssembly == null || codeDomParameters.OutputAssembly.Length == 0))
		{
			string text = AssemblyNameFromOptions(codeDomParameters.CompilerOptions);
			if (text == null)
			{
				text = GetTempAssemblyPath(codeDomParameters.TempFiles.TempDir, parentAssemblies, ns);
			}
