...
internal class XmlSerializationReaderCodeGen : XmlSerializationCodeGen
{
	...
	private void WriteLiteralStructMethod(StructMapping structMapping)
	{
		...
		string unknownTypeName = GetUnknownTypeName(structMapping);
		if (unknownTypeName != null)
		{
			WriteDeclareMapping(structMapping, "o", false);
			base.Writer.WriteLine("bool isUnknownType = false;");
		}
		...
		else if (unknownTypeName != null)
		{
			...
			{
				WriteConstructMapping(structMapping, "o");
			}
			base.Writer.Indent--;
		}
		base.Writer.WriteLine("}");
		...
		if (unknownTypeName != null)
		{
			...
			WriteConstructMapping(structMapping, "o", unknownTypeName, false);
			base.Writer.WriteLine("isUnknownType = true;");
			base.Writer.Indent--;
			base.Writer.WriteLine("}");
		}
		else
		...
		if (!structMapping.TypeDesc.IsAbstract)
		{
			...
			if (unknownTypeName == null)
			{
				WriteDeclareMapping(structMapping, "o");
			}
			WriteConstructMapping(structMapping, "o");
			if (unknownTypeName != null)
			{
		...
		else
		{
			...
			else if (structMapping.TypeDesc.IsNullable)
			{
				base.Writer.WriteLine("if (isNull) return null;");
			}
			base.Writer.Write("throw CreateAbstractTypeException(");
		...
		if (unknownTypeName != null || !structMapping.TypeDesc.IsAbstract)
		{
			...
			WriteAttributes(members3, member3, "UnknownNode", "(object)o", (unknownTypeName != null) ? ("(" + typeof(IUnknownType).FullName + ")o") : null);
			if (member3 != null)
			{
			...
			int loopIndex = WriteWhileNotLoopStart();
			base.Writer.Indent++;
			string unknownNode = "UnknownNode((object)o, " + ExpectedElements(members3) + ");";
			WriteMemberElements(members3, delegate
			{
				if (unknownTypeName != null)
				{
					base.Writer.Write("if (isUnknownType) UnknownTypeNode((");
					base.Writer.Write(typeof(IUnknownType).FullName);
					base.Writer.Write(")o);");
					base.Writer.Write("else ");
				}
				base.Writer.WriteLine(unknownNode);
			}, unknownNode, member2, member, null);
			base.Writer.WriteLine("Reader.MoveToContent();");
			WriteWhileLoopEnd(loopIndex);
			WriteMemberEnd(members2);
			base.Writer.WriteLine("ReadEndElement();");
	...
	private void WriteAddCollectionFixup(TypeDesc typeDesc, bool readOnly, string memberSource, string targetSource)
	{
		...
		{
			createCollectionInfo = new CreateCollectionInfo("create" + (++nextCreateMethodNumber).ToString(CultureInfo.InvariantCulture) + "_" + typeDesc.Name, typeDesc);
	...
	private void WriteAttributes(Member[] members, Member anyAttribute, string elseCall, string firstParam, string unknownTypeCast = null)
	{
		int num = 0;
		Member member = null;
		ArrayList arrayList = new ArrayList();
		...
		if (unknownTypeCast != null)
		{
			base.Writer.WriteLine("if (isUnknownType)");
			base.Writer.Indent++;
			base.Writer.Write("UnknownTypeAttribute(");
			base.Writer.Write(unknownTypeCast);
			base.Writer.WriteLine(");");
			base.Writer.Indent--;
			base.Writer.Write("else ");
		}
	...
	{
		WriteMemberElements(members, delegate
		{
			base.Writer.WriteLine(elementElseString);
		}, elseString, anyElement, anyText, checkTypeHrefsSource);
	}

	private void WriteMemberElements(Member[] members, Action elementElseAction, string elseString, Member anyElement, Member anyText, string checkTypeHrefsSource)
	{
		...
		if (num)
		{
			WriteIfNotSoapRoot(delegate
			{
				elementElseAction();
				base.Writer.WriteLine("continue;");
			});
			WriteMemberElementsCheckType(checkTypeHrefsSource);
		}
		else
		{
			WriteMemberElementsIf(members, anyElement, elementElseAction, null);
		}
		base.Writer.Indent--;
		base.Writer.WriteLine("}");
		if (anyText != null)
	...
	private void WriteMemberElementsElse(Member anyElement, Action elementElseAction)
	{
		if (anyElement != null)
		{
	...
	{
		WriteMemberElementsIf(members, anyElement, delegate
		{
			base.Writer.WriteLine(elementElseString);
		}, checkTypeSource);
	}

	private void WriteMemberElementsIf(Member[] members, Member anyElement, Action elementElseAction, string checkTypeSource)
	{
		...
		WriteMemberElementsElse(anyElement, elementElseAction);
		if (num > 0)
		{
			if (flag2)
			{
	...
	{
		base.Writer.WriteLine(string.Format(CultureInfo.InvariantCulture, "int whileIterations{0} = 0;", new object[1] { nextWhileLoopIndex }));
		base.Writer.WriteLine(string.Format(CultureInfo.InvariantCulture, "int readerCount{0} = ReaderCount;", new object[1] { nextWhileLoopIndex }));
		return nextWhileLoopIndex++;
	}

	private void WriteWhileLoopEndCheck(int loopIndex)
	{
		base.Writer.WriteLine(string.Format(CultureInfo.InvariantCulture, "CheckReaderCount(ref whileIterations{0}, ref readerCount{1});", new object[2] { loopIndex, loopIndex }));
	}
	...
	private void WriteIfNotSoapRoot(Action sourceAction)
	{
	...
	private void WriteDeclareMapping(TypeMapping mapping, string local, bool? useReflection = null)
	{
		string cSharpName = mapping.TypeDesc.CSharpName;
		base.Writer.Write((useReflection ?? mapping.TypeDesc.UseReflection) ? "object" : cSharpName);
		base.Writer.Write(" ");
		base.Writer.Write(local);
		base.Writer.WriteLine(";");
	}

	private void WriteConstructMapping(TypeMapping mapping, string local, string fullTypeName = null, bool? useReflection = null)
	{
		if (fullTypeName == null)
		{
			fullTypeName = mapping.TypeDesc.CSharpName;
		}
		bool flag = useReflection ?? mapping.TypeDesc.CannotNew;
		if (flag)
		{
			base.Writer.WriteLine("try {");
			base.Writer.Indent++;
		...
		base.Writer.Write(local);
		base.Writer.Write(" = ");
		base.Writer.Write(base.RaCodeGen.GetStringForCreateInstance(fullTypeName, useReflection ?? mapping.TypeDesc.UseReflection, flag, cast: true));
		base.Writer.WriteLine(";");
		if (flag)
		{
	...
	{
		WriteDeclareMapping(mapping, local);
		WriteConstructMapping(mapping, local);
	}
