...
public abstract class XmlSerializationWriter : XmlSerializationGeneratedCode
{
	...
	protected ArrayList Namespaces
	{
		get
		{
			if (namespaces != null)
			{
				return namespaces.get_NamespaceList();
			}
			return null;
		}
		set
	...
	protected void WriteUnknownTypeAttributes(IUnknownType unknownType)
	{
		if (unknownType.Attributes == null)
		{
			return;
		}
		foreach (KeyValuePair<string, string> attribute in unknownType.Attributes)
		{
			WriteAttribute(attribute.Key, attribute.Value);
		}
	}

	protected void WriteUnknownTypeNodes(IUnknownType unknownType)
	{
		if (unknownType.Nodes == null)
		{
			return;
		}
		foreach (XmlNode node in unknownType.Nodes)
		{
			node.WriteTo(w);
		}
	}
	...
	protected void WriteStartElement(string name, string ns, object o, bool writePrefixed, XmlSerializerNamespaces xmlns)
	{
		...
		if (namespaces != null)
		{
			foreach (string key in namespaces.get_Namespaces().Keys)
			{
				string text3 = (string)namespaces.get_Namespaces()[key];
			...
			usedPrefixes = ListUsedPrefixes(namespaces.get_Namespaces(), aliasBase);
		}
		...
		if (namespaces != null)
		{
			foreach (string key2 in namespaces.get_Namespaces().Keys)
			{
				string text5 = (string)namespaces.get_Namespaces()[key2];
	...
	protected void WriteNamespaceDeclarations(XmlSerializerNamespaces xmlns)
	{
		if (xmlns != null)
		{
			foreach (DictionaryEntry item in xmlns.get_Namespaces())
			{
				string text = (string)item.Key;
				string text2 = (string)item.Value;
				if (namespaces != null && namespaces.get_Namespaces()[text] is string text3 && text3 != text2)
				{
