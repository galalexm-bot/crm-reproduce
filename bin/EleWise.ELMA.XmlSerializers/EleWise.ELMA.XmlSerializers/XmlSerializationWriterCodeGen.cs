...
internal class XmlSerializationWriterCodeGen : XmlSerializationCodeGen
{
	...
	private void WriteDerivedTypes(StructMapping mapping)
	{
		for (StructMapping structMapping = mapping.DerivedMappings; structMapping != null; structMapping = structMapping.NextDerivedMapping)
		{
			...
			if (GetUnknownTypeName(structMapping) != null)
			{
				base.Writer.Write(" || isUnknownType && ");
				WriteTypeCompare("t.BaseType", cSharpName, structMapping.TypeDesc.UseReflection);
			}
			base.Writer.WriteLine(") {");
			base.Writer.Indent++;
	...
	private void WriteStructMethod(StructMapping mapping)
	{
		...
		base.Writer.Write(s);
		string cSharpName = mapping.TypeDesc.CSharpName;
		string unknownTypeName = GetUnknownTypeName(mapping);
		if (mapping.IsSoap)
		{
		...
		else
		{
			...
			base.Writer.Write("bool isUnknownType = o is ");
			...
			{
				base.Writer.Write(" || isUnknownType && ");
				WriteTypeCompare("t.BaseType", cSharpName, mapping.TypeDesc.UseReflection);
			}
			base.Writer.WriteLine(") {");
			base.Writer.WriteLine("}");
		...
		if (!mapping.TypeDesc.IsAbstract)
		{
			...
			if (!mapping.IsSoap)
			{
				...
				if (!mapping.TypeDesc.IsRoot)
				{
					if (unknownTypeName != null)
					{
						base.Writer.Write("if (o is ");
						base.Writer.Write(typeof(IUnknownType).FullName);
						base.Writer.Write(") WriteUnknownTypeAttributes((");
						base.Writer.Write(typeof(IUnknownType).FullName);
						base.Writer.Write(")o);");
			...
			if (unknownTypeName != null)
			{
				base.Writer.Write("if (o is ");
				base.Writer.Write(typeof(IUnknownType).FullName);
				base.Writer.Write(") WriteUnknownTypeNodes((");
				base.Writer.Write(typeof(IUnknownType).FullName);
				base.Writer.WriteLine(")o);");
			}
			if (!mapping.IsSoap)
