...
public class AddContactBtn : IActionItemProvider
{
	public IEnumerable<IActionItem> GetItems(IActionItem rootItem, HtmlHelper htmlHelper)
	{
		if (htmlHelper.get_ViewData().get_Model() is ContactModel contactModel)
		{
			yield return new ActionToolbarItem
			{
				Url = htmlHelper.Url().Action("GetForContact", "VcardCRM", (object)new
				{
					area = "EleWise.ELMA.vCard.Web",
					id = contactModel.Entity.Uid
				}),
				IconUrl = "#vCard.svg",
	...
	public void InsertItems(IActionItem rootItem, HtmlHelper htmlHelper)
	{
		...
		int index = -1;
		IActionItem actionItem = moreButton.Items.FirstOrDefault((IActionItem i) => i.Uid == "Contact_delete_btn");
		if (actionItem != null)
		{
		...
		{
			IActionItem actionItem2 = moreButton.Items.FirstOrDefault((IActionItem i) => i.Uid == "Contact_delete_separator");
			if (actionItem2 != null)
			{
