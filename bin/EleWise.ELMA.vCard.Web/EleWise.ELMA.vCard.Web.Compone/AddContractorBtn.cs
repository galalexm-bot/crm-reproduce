...
public class AddContractorBtn : IActionItemProvider
{
	public IEnumerable<IActionItem> GetItems(IActionItem rootItem, HtmlHelper htmlHelper)
	{
		if (htmlHelper.get_ViewData().get_Model() is ContractorLegalModel contractorLegalModel)
		{
			yield return new ActionToolbarItem
			{
				Url = htmlHelper.Url().Action("GetForContractor", "VcardCRM", (object)new
				{
					area = "EleWise.ELMA.vCard.Web",
					id = contractorLegalModel.Contractor.Uid
				}),
				IconUrl = "#vCard.svg",
	...
	public void InsertItems(IActionItem rootItem, HtmlHelper htmlHelper)
	{
		...
		{
			IActionItem actionItem2 = moreButton.Items.FirstOrDefault((IActionItem i) => i.Uid == "Contractor_delete_separator");
			if (actionItem2 != null)
			{
